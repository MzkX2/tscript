...
import tscript.base.Const;
import tscript.base.Const.CType;
...
class Parser {
	...
	**/
	...
	public function parseString(s:String, ?origin:String = "TScript") {
		initParser(origin);
		input = s;
		readPos = 0;
		var a = new Array();
	...
	function interpolateString(s:String) {
		if (s == null)
			error(ECustom("Null Object Reference"));
	...
	function parseLambda(args:Array<Defs.Argument>, pmin) {
		while (true) {
			var id = getIdent();
	...
	function parseStructure(id) {
		...
		return switch (id) {
					...
					switch tk {
						case TId(s):
							if (s == "from") {
								...
								var tk = token();
								push(tk);
								if (Type.enumEq(tk, TId("from")))
									unexpected(TId("from"));
							} else if (s == "to") {
								hasTo = true;
								var tk = token();
								push(tk);
								if (Type.enumEq(tk, TId("to")))
									unexpected(TId("to"));
							}
							if (!hasFrom && !hasTo) unexpected(tk);
						case TBrOpen:
							var tk = token();
							if (tk == TBrClose) break; else {
								push(tk);
								while (true) {
									var tk = token();
									if (tk != TStatement && tk != TBrClose && tk != TEof)
										push(tk);
									if (tk == TBrClose || tk == TEof)
										break;
					...
					case TId(s):
						switch s {
							...
							case "public" | "private": // needs checking
								if ((s == "public" && inPublic) || (s == "private" && inPrivate))
									unexpected(tk);
			...
			case "return":
				...
				if (tk == TStatement) mk(EReturnEmpty, p1, tokenMax); else mk(EReturn(e), p1, if (e == null) tokenMax else pmax(e));
			case "new":
				var a = new Array();
	...
	function parseFunctionArgs() {
		...
		if (tk != TPClose) {
			...
			while (!done) {
				...
				var arg:Defs.Argument = {name: name};
				args.push(arg);
				if (opt)
					arg.opt = true;
				if (allowTypes) {
					...
					if (maybe(TOp("="))) {
						...
						switch arg.value.e {
							...
							case EIdent("null"):
							case _:
								error(ECustom('Default argument value should be constant'));
						}
						// arg.opt = true;
					}
				}
	...
	function parseType():Const.CType {
		var t = token();
		switch (t) {
			case TId(v):
				push(t);
	...
	function parseTypeNext(t:Const.CType) {
		var tk = token();
		switch (tk) {
			case TOp(op):
				if (op != "->") {
		...
		while (true) {
			...
			switch (char) {
				...
				case "]".code:
					return TBkClose;
				case "'".code, '"'.code:
					return TConst(CString(readString(char), char == 39));
				case "?".code:
					char = readChar();
					if (char == '.'.code)
						return TQDot;
	...
	static function constString(c) {
		return switch (c) {
			...
			#if !haxe3
			case Const.CInt32(v): Std.string(v);
			#end
		}
	}
