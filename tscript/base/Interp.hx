...
private enum TScriptNull {
	Not_NULL;
}

@:keepSub
@:access(tscript.base.EClass)
@:access(tscript.base.Parser)
@:access(tscript.TScript)
class Interp {
	static var eabstracts:Array<String> = [];
	static var EABSTRACTS:Map<String, {?fileName:String, base:EAbstract}> = [];
	static var classes:Array<String> = [];
	static var STATICPACKAGES:Map<String, {?fileName:String, base:EClass}> = [];
	...
	public function posInfos():PosInfos {
		...
		return cast {fileName: "TScript", lineNumber: 0};
	}
	...
	inline function error(e:Error.ErrorDef, rethrow = false):Dynamic {
		if (resumeError)
			return null;
		if (curExpr == null)
			curExpr = {
	...
	function resolve(id:String):Dynamic {
		...
		if (STATICPACKAGES.exists(id))
			return STATICPACKAGES[id].base;
		else if (EABSTRACTS.exists(id))
			return canUseAbs ? EABSTRACTS[id].base : error(ECannotUseAbs);
	...
	public function expr(e:Expr):Dynamic {
		...
		switch (e) {
			case EEAbstract(ident, parent, exprs, fromParent):
				if (eabstracts.contains(ident) || classes.contains(ident)) {
					...
					if (EABSTRACTS[ident] != null)
						baseData = EABSTRACTS[ident].fileName;
					else if (STATICPACKAGES[ident] != null)
						baseData = STATICPACKAGES[ident].fileName;
					error(EAlreadyModule(ident, baseData));
				...
				var eabstract = new EAbstract(ident);
				for (i in exprs) {
					var isPublic = true;
					var evar = null;
					switch i.e {
					...
					if (evar != null) {
						...
						var sugar = EAbstract.create(isPublic, name, value);
						if (eabstract.fields.exists(name))
							error(ECustom("Duplicate abstract field declaration : " + ident + "." + name));
						if (parent == "Int") {
							if (value == null && reallyNull) {
								sugar.value = abstractValueInt + 1;
								abstractValueInt++;
							} else if (value != null && abstractValueInt != value && !reallyNull)
								abstractValueInt = sugar.value;
							else if (!(value != null && abstractValueInt == value))
								abstractValueInt++;
			...
			case EClass(cl, e):
				...
				@:privateAccess script.setClassPath(cl);
				curClass = cl;
				var v = null;
				if (!STATICPACKAGES.exists(cl)) {
					...
					v = new EClass(cl);
					STATICPACKAGES.set(cl, {fileName: fileName, base: v});
				}
				for (e in e)
					switch e.e {
						case _: expr(e);
			...
			case EVar(n, f, t, e):
				...
				if (!inStatic) {
					...
					if (script.classPath != null && script.classPath.length > 0 && STATICPACKAGES.exists(script.classPath))
						STATICPACKAGES.get(script.classPath)
							.base.fields.set(n, tscript.base.EClass.create(inPublic && !noPrivateAccess, expr1, f, noPrivateAccess, false));
					else if (inPublic) {
						if (!pushedVars.contains(n)) {
							pushedVars.push(n);
							TScript.globalVariables.set(n, expr1);
						}
					}
				}
			...
			case EWhile(econd, e):
				if (strictVar)
					return error(EUnexpected("while"));
				whileLoop(econd, e);
				return null;
			case EDoWhile(econd, e):
				if (strictVar)
					return error(EUnexpected("do"));
				doWhileLoop(econd, e);
				return null;
			case EFor(v, v2, it, e):
				forLoop(v, v2, it, e);
			...
			case EImport(e, c, asIdent, f):
				...
				if (e == null && STATICPACKAGES.exists(og)) {
					f = null;
					for (i => k in STATICPACKAGES[og].base.fields)
						variables.set(i, k.value);
				}
				if (e == null && f != null)
				...
				if (name != null) {
					if (depth == 0) {
						...
						if (inStatic) {
							if (script.classPath != null && script.classPath.length > 0 && STATICPACKAGES.exists(script.classPath))
								STATICPACKAGES.get(script.classPath)
									.base.fields.set(name, tscript.base.EClass.create(inPublic && !noPrivateAccess, f, false, false, true));
							else if (inPublic) {
								if (!pushedVars.contains(name)) {
									pushedVars.push(name);
									TScript.globalVariables.set(name, f);
								}
							}
						}
					} else {
	...
	function checkBool(ec:Dynamic, type = "while"):Void {
		if (ec != null && !Std.isOfType(ec, Bool)) {
			...
			if (n != null)
				error(ECustom(n + ' should be Bool'));
			else
				error(ECustom('Invalid $type expression (should be Bool)'));
		}
	}
	...
	function get(o:Dynamic, f:String):Dynamic {
		...
		return {
			for (i => k in STATICPACKAGES)
				if (k.base == o) {
					var v = k.base.fields.get(f);
					if (v == null)
						error(EDoNotHaveField(k.base, f));
			...
			for (i => k in EABSTRACTS)
				if (k.base == o) {
					var v = k.base.fields[f];
					if (v == null)
						error(EAbstractField(k.base, f));
